...
public class SgChestManager {
	...
	public void putTir2Item(Inventory i) {
		ItemStack it;
		int put;
		for (int a = 0; a < (new Random().nextInt(3) + 4); a++) {
			do {
				it = SgChestManager.tir2Items.get(new Random().nextInt(SgChestManager.tir2Items.size()));
				put = new Random().nextInt(i.getSize());
	...
	public void putTir1Item(Inventory i) {
		ItemStack it;
		int put;
		for (int a = 0; a < (new Random().nextInt(3) + 3); a++) {
			do {
				it = SgChestManager.tir1Items.get(new Random().nextInt(SgChestManager.tir1Items.size()));
				put = new Random().nextInt(i.getSize());
			} while (i.getItem(put) != null || i.contains(it));
			i.setItem(put, it);
		}
		for (int a = 0; a < (new Random().nextInt(2) + 2); a++) {
			do {
				it = SgChestManager.tir1Tools.get(new Random().nextInt(SgChestManager.tir1Tools.size()));
				put = new Random().nextInt(i.getSize());
			} while (i.getItem(put) != null || i.contains(it));
			i.setItem(put, it);
		}
	...
	public void putCenteritem(Inventory i) {
		ItemStack it;
		int put;
		int size = SgChestManager.centerItems.size();
		for (int a = 0; a < (new Random().nextInt(4) + 3); a++) {
			do {
				it = centerItems.get(new Random().nextInt(size));
				put = new Random().nextInt(i.getSize());
			} while (i.getItem(put) != null || i.contains(it));
			i.setItem(put, it);
		}
