...
public class Lang {
	...
	public void setupString() {
		outArena = Options.cc(prefix + "&c&l你已經超出&f&l場地限制範圍!!&c&l請趕快回場地!!!");
		cantStart = Options.cc(prefix + "&c遊戲人數不足造成無法開始");
		needDeathMatch = Options.cc(prefix + "&c&l由於人數剩下&f&l4人!&c&l因此加快遊戲速度!&f&l2分鐘後便要&c&l死亡之戰!");
		gameStarted = Options.cc(prefix + "&f&l遊戲開始了,祝福各位&6&l好運");
		allLightStart = Options.cc(prefix + "&f&l現在開始你不管在哪裡都會受到&6&l傷害!&f&l請趕快擊殺對手!");

		lobbyCount = Options.cc(prefix + "&f即將在&a <time> &f秒後傳送玩家");
		startCount = Options.cc(prefix + "&6遊戲即將在&f <time> &6後開始");
		gameCount = Options.cc(prefix + "&c死亡之戰倒數&f <time>");
		deathMatchStartCount = Options.cc(prefix + "&c玩家即將在 <time> 秒後釋放");
		allStrikeLightCount = Options.cc(prefix + "&c&l極限淘汰模式即將在 &f&l<time> &c&l秒後開始");

		youGotKillByS1 = Options.cc(prefix + "&f你被&c <player> &f以&b <heart>&c\u2764 &f差距擊殺了");
		youKills1 = Options.cc(prefix + "&f你擊殺了&6 <player>!");
		s1Death = Options.cc(prefix + "&c<player> &f死亡了!遊戲還剩下&6 <size> &f人存活!");

		pplJoin = Options.cc(prefix + "&6<player> &f加入了&6Mitw&eSG &7(<now>/&c24&7)");
		pplLeave = Options.cc(prefix + "&6<player> &7離開了遊戲 &7(<now>/&c24&7)");
	...
	public void setupItems() {
		...
		iVoteMap = SurvivalGames.getItemBuilder().itemWithName(Material.PAPER, Options.cc("&6&l投票&f&l地圖"));
		arrowTrails = SurvivalGames.getItemBuilder().itemWithName(Material.CHEST, Options.cc("&f&l選擇&e&l箭矢特效"));
	...
	public void setupList() {
		...
		victoryMsg.add("§6§m--------------------------");
		victoryMsg.add("");
		victoryMsg.add(Options.cc("§f§l恭喜&6&l <player> &f&l獲得勝利"));
		victoryMsg.add("");
		victoryMsg.add("§6§m--------------------------");
		...
		infoMessage.add(Options.cc("              &6&l歡迎來到MitwSG"));
		infoMessage.add(Options.cc("           &f盡你所能的搶到最多箱子"));
		infoMessage.add(Options.cc("&f可以的話,嘗試到隱密的地方搜尋&e高等箱"));
		infoMessage.add(Options.cc("              &c&l並且隨時注意身旁"));
		infoMessage.add(Options.cc("                &a&l目標獲得勝利!"));
		infoMessage.add(Options.cc("&f&m-----------------------------"));
	}
